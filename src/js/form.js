...
function initSrc() {
	regUrl = 'https://ielts.etest.net.cn/login';
	btcUrl = 'https://ielts.etest.net.cn/login';
	logoutUrl = 'https://ielts.etest.net.cn/login';
...
function start() {
	...
	id = setInterval(function () {
		...
		if (title == '雅思网上报名 - 教育部考试中心') {

			var nineM = iframe.contentDocument.getElementById('2018-09');
			var fujian = iframe.contentDocument.getElementById('mvfSiteProvinces235');
			var zhejiang = iframe.contentDocument.getElementById('mvfSiteProvinces233');
			var btnSearch = iframe.contentDocument.getElementById('btnSearch');
			var btnBackToSearch = iframe.contentDocument.getElementById('btnBackToSearch');
			var adminDate2018 = iframe.contentDocument.getElementById('adminDate2018-09-28T16');
			

			if (adminDate2018) {
				if (adminDate2018.parentNode.parentNode.parentNode.nextSibling.childNodes[0].cells[5].innerText == '有名额') {
					console.log('有名额');
					alert('有名额，赶紧报名');
				} else {
					btnBackToSearch.click();
				}
			} else if (nineM && nineM.value != '') {
				nineM.checked = true;
				fujian.checked = true;
				// zhejiang.checked = true;
				btnSearch.click();
			} else {
